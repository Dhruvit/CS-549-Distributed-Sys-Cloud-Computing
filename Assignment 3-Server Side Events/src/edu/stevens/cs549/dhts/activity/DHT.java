...
public class DHT extends DHTBase implements IDHTResource, IDHTNode, IDHTBackground {
	...
	private NodeInfo getSucc(NodeInfo info) throws Failed {
		...
		if (localInfo.addr.equals(info.addr)) {
			...
			// TODO: Do the Web service call. Add by Dhruvit
...
//		routing.joinCheck();
	...
	protected NodeInfo closestPrecedingFinger(NodeInfo info, int id) throws Failed {
		...
		if (localInfo.equals(info)) {
			...
			if (IRouting.USE_FINGER_TABLE) {
				/*
				 * TODO: Do the Web service call to the remote node. Add by Dhruvit
				 */
	...
	public EventOutput listenForBindings(int id, String key) {
		// TODO create event output stream and add to broadcaster. add by Dhruvit
	...
	public void stopListening(int id, String key) {
		// TODO remove event output stream from broadcaster. add by Dhruvit.
